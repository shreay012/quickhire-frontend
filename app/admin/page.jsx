...
export default function AdminDashboard() {
  ...
  // This page is rendered both at `/admin` (root) and at `/admin/[country]/`
  // via re-export. When a country segment is present we keep nav links
  // inside the country prefix so the breadcrumb / URL reflects the scope.
  const params = useParams();
  const country = params?.country ? String(params.country).toLowerCase() : '';
  const base = country ? `/admin/${country}` : '/admin';
      ...
      <div className="p-4 sm:p-8 space-y-6">
        ...
        <Card className="px-5 sm:px-6 py-5 bg-gradient-to-r from-[#F2F9F1] via-white to-white">
          <div className="flex items-start sm:items-center justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <Button variant="primary" size="md" onClick={() => router.push(`${base}/bookings`)}>
                Open bookings
              </Button>
              <Button variant="subtle" size="md" onClick={() => router.push(`${base}/cms/banners`)}>
                Edit banners
              </Button>
            </div>
          </div>
            ...
            {stats && (
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <StatCard
                  ...
                  icon="⏳"
                  onClick={() => router.push(`${base}/bookings?status=pending`)}
                />
              </div>
            )}
          </>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <SectionCard
            ...
            action={
              <Button size="sm" variant="subtle" onClick={() => router.push(`${base}/analytics`)}>
                Full analytics
              </Button>
            }
          >
          ...
          <SectionCard
            ...
            action={
              <Button size="sm" variant="subtle" onClick={() => router.push(`${base}/bookings`)}>
                Open list
              </Button>
            }
          >
                ...
                {Object.entries(bookingsByStatus)
                  ...
                  .map(([status, count]) => (
                    <button
                      key={status}
                      type="button"
                      onClick={() => router.push(`${base}/bookings?status=${status}`)}
            ...
            <div className="divide-y divide-[#EEF5EC] -mx-5 sm:-mx-5">
              {recentActivity.map((item) => (
                <button
                  key={item._id}
                  type="button"
                  onClick={() => router.push(`${base}/bookings/${item._id}`)}
